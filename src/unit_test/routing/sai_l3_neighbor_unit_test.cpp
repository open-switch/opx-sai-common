                                            ...
                                            const char *p_ip_addr_str)
{
    ...
    EXPECT_EQ (SAI_NEXT_HOP_TYPE_IP, attr_list[0].value.s32);
...
/*
 * Validates Neighbor creation and removal with SAI_NEIGHBOR_ENTRY_ATTR_PACKET_ACTION
 * attribute set.
 */
...
/*
 * Validate Neighbor creation and removal when nexthop exists for the IP
 * address.
 */
TEST_F (saiL3NeighborTest, create_and_remove_when_nexthop_exists)
{
    sai_status_t          status;
    const char           *p_ip_addr_str = "11.0.0.1";
    sai_ip_addr_family_t  ip_af = SAI_IP_ADDR_FAMILY_IPV4;
    ...
    /* Create the next hop object first */
    ...
    /* Verify the next hop object still exists */
    sai_verify_ip_nh_after_neighbor_removal (nh_id, port_rif_id, ip_af,
                                             p_ip_addr_str);
...
/*
 * Validate setting SAI_NEIGHBOR_ENTRY_ATTR_DST_MAC_ADDRESS for an existing
 * neighbor.
 */
...
/*
 * Validate setting SAI_NEIGHBOR_ENTRY_ATTR_PACKET_ACTION for an existing neighbor.
 */
 ...
 * SAI_NEIGHBOR_ENTRY_ATTR_PACKET_ACTION attribute returns appropriate error status.
 */
...
/*
 * Validate getting SAI_NEIGHBOR_ENTRY_ATTR_DST_MAC_ADDRESS is successful, even
 * when the SAI_NEIGHBOR_ENTRY_ATTR_PACKET_ACTION is set to TRAP or DROP.
 */
...
/*
 * Validate creating VLAN based neighbor entry with drop action.
 */
TEST_F (saiL3NeighborTest, drop_action_on_vlan_rif)
{
    sai_status_t          status;
    const char           *p_ip_addr_str = "11.0.0.1";
    sai_ip_addr_family_t  ip_af = SAI_IP_ADDR_FAMILY_IPV4;
    ...
    /* Create Neighbor with DROP action */
    status = sai_test_neighbor_create (vlan_rif_id, ip_af, p_ip_addr_str,
                                       default_neighbor_attr_count,
                                       SAI_NEIGHBOR_ENTRY_ATTR_PACKET_ACTION,
    ...
    /* Set FORWARD action without the mac address attribute */
    status = sai_test_neighbor_attr_set (vlan_rif_id, ip_af, p_ip_addr_str,
                                         SAI_NEIGHBOR_ENTRY_ATTR_PACKET_ACTION,
                                         SAI_PACKET_ACTION_FORWARD, NULL);
    ...
    sai_neighbor_verify_after_creation (vlan_rif_id, ip_af, p_ip_addr_str,
                                        p_mac_str, SAI_PACKET_ACTION_FORWARD);
